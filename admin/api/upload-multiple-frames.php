...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $namePrefix = $_POST['name_prefix'] ?? 'Frame';
    ...
    for ($i = 0; $i < $fileCount; $i++) {
        ...
        $uniqueFileName = 'frame_' . uniqid() . '_' . time() . '.' . $extension;
    ...
    if ($response['uploaded'] > 0 && $response['failed'] == 0) {
        $response['message'] = "Successfully uploaded {$response['uploaded']} frames";
    } elseif ($response['uploaded'] > 0 && $response['failed'] > 0) {
        $response['message'] = "Uploaded {$response['uploaded']} frames, {$response['failed']} failed";
    } else {
