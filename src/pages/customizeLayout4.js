document.addEventListener('DOMContentLoaded', function() {
        ...
        .then(data => {
            if (data.success && data.photos) {
                ...
                if (imageArrayLength === 0) {
                    console.error('No images found in server session for Layout 4');
    ...
    function redrawCanvas() {
        console.log(`Redrawing Layout 4 canvas with background: ${backgroundType}`);
        ...
        // For Layout 4 with 8 photos, arrange them in a 4x2 grid (4 columns, 2 rows)
        const photoWidth = (availableWidth - (photoGap * 3)) / 4; // 4 columns with gaps
        ...
        function drawPhotos() {
            let loadedCount = 0;
            const totalPhotos = Math.min(storedImages.length, 8); // Max 8 photos for Layout 4

            // Draw all 8 photos in 4x2 grid
            for (let i = 0; i < totalPhotos; i++) {
                if (storedImages[i]) {
                    ...
                    img.onload = () => {
                        ...
                        const row = Math.floor(i / 4); // Row (0 or 1)
                    ...
                    // When all 8 photos are loaded, add stickers, text, and logo
