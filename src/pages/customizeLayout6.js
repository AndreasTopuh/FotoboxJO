document.addEventListener('DOMContentLoaded', function() {
    ...
    const storedImages = JSON.parse(sessionStorage.getItem('photoArray6')) || [];
    const imageArrayLength = storedImages.length; // Should be 4 for Layout 6
    ...
    if (imageArrayLength === 0) {
        ...
        window.location.href = 'canvasLayout6.php';
        return;
    }

    // 4R Canvas dimensions - Optimized for web and print
    function redrawCanvas() {
        console.log(`Redrawing Layout 6 canvas with background: ${backgroundType}`);
        ...
        // For Layout 6 with 4 photos, arrange them in a 2x2 grid
        ...
        function drawPhotos() {
            let loadedCount = 0;
            const totalPhotos = Math.min(storedImages.length, 4); // Max 4 photos for Layout 6
            ...
            for (let i = 0; i < totalPhotos; i++) {
                if (storedImages[i]) {
                    ...
                    img.onload = () => {
                        ...
                        const row = Math.floor(i / 2); // Row (0 or 1)
                    ...
                    // When all 4 photos are loaded, add stickers, text, and logo
