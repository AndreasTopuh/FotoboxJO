/*
 * ⚡ CUSTOMIZE LAYOUT 2 - LAYOUT-SPECIFIC FRAMES VERSION
 * ✅ Updated: Uses frames specific to Layout 2 only
 * 📝 API: /src/api-fetch/get-frames-by-layout.php?layout_id=2
 */
document.addEventListener('DOMContentLoaded', () => {
  // ⚡ COMPRESSION CONFIGURATION - ENHANCED & SYNCHRONIZED Quality System
  // ✅ SYNCHRONIZED dengan canvasLayout2.js untuk konsistensi kualitas end-to-end
  const COMPRESSION_CONFIG = {
    // Untuk session storage (temporary) - HIGH QUALITY untuk hasil yang konsisten
    SESSION_QUALITY: 0.9,        // 90% - selaras dengan canvasLayout2.js
    ...
    DOWNLOAD_QUALITY: 0.98,      // 98% - selaras dengan canvasLayout2.js
    DOWNLOAD_MAX_WIDTH: 3000,    // Full resolution untuk print berkualitas
    DOWNLOAD_MAX_HEIGHT: 3600,   // Layout 2 dapat resolution tinggi seperti Layout 1

    // Untuk preview thumbnail - BALANCED PREVIEW
    THUMB_QUALITY: 0.8,          // 80% - tetap bagus untuk preview
    THUMB_MAX_WIDTH: 600,        // Naikan selaras dengan canvasLayout2.js
    THUMB_MAX_HEIGHT: 800        // Naikan selaras dengan canvasLayout2.js
  };
      ...
      img.onload = () => {
        try {
          ...
          console.log(`✅ Image compressed for ${mode}:`, {
            ...
            reduction: `${Math.round((1 - compressedDataUrl.length / imageData.length) * 100)}%`
          });
  ...
  // Asset Loading
  /**
   * Loads frames and stickers from database - Layout 2 specific
   */
  ...
  async function redrawCanvas() {
    ...
    // ✅ ENHANCED PREVIEW SCALING - Same as Layout 1
  ...
  async function drawStickersAndLogos(ctx, canvas) {
    // Priority: Frame & Sticker Combo > Regular Sticker
  ...
  function updateCanvasPreview(canvas) {
    if (DOM.photoCustomPreview) {
      ...
      Object.assign(canvas.style, {
        // ✅ ENHANCED PREVIEW SIZE - Same as Layout 1 for consistency
  ...
  async function generateHighQualityCanvas() {
    ...
    // ✅ ENHANCED: 3x scale untuk maximum print quality (same as Layout 1)
        ...
        printWindow.document.write(`
          ...
          <body>
            <div class="print-container">
              <img src="${imageDataUrl}" class="print-image" alt="Print Photo - High Quality" />
            </div>
          </body>
          </html>
        `);
        ...
        printWindow.onload = () => {
          setTimeout(() => {
            ...
            if (printBtnEl) {
              ...
              printBtnEl.style.opacity = '0.5';
              printBtnEl.style.cursor = 'not-allowed';
              printBtnEl.innerHTML = '✅ Sudah Print (HQ)';
            }
