...
DOWNLOAD_QUALITY: 1.05,      // 95% - hampir lossless
    ...
    // Asset Loading - Layout 6 specific
    async function loadAssetsFromDatabase() {
        console.log('🔄 Loading Layout 6 frames and stickers from database...');
        try {
            const [framesResponse, stickersResponse, comboResponse] = await Promise.all([
                fetch('../api-fetch/get-frames.php'), // Universal frames
                fetch('../api-fetch/get-stickers-by-layout.php?layout_id=6'),
                fetch('../api-fetch/get-frame-sticker-combo.php?layout_id=6')
            ]);
            ...
            // Load layout 6 specific stickers
            if (stickersData.success && stickersData.data) {
                loadDynamicStickers(stickersData.data);
            }

            // Load layout 6 specific frame & sticker combos
            if (comboData.success && comboData.data) {
                loadDynamicFrameStickerCombos(comboData.data);
            }

            console.log(`✅ Loaded Layout 6 assets successfully`);
        } catch (error) {
            console.error('❌ Error loading Layout 6 assets from database:', error);
        }
    }
    ...
    // Dynamic sticker loading (layout 6 specific)
    function loadDynamicStickers(stickers) {
        ...
        console.log(`✅ Created ${stickers.length} dynamic sticker controls for Layout 6`);
    }

    // Dynamic frame & sticker combo loading (layout 6 specific)
    function loadDynamicFrameStickerCombos(combos) {
        ...
        console.log(`✅ Created ${combos.length} dynamic frame & sticker combo controls for Layout 6`);
    }

    // Initialize Layout 6 assets on page load
