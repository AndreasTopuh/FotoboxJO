document.addEventListener('DOMContentLoaded', () => {
    // 🚀 COMPRESSION CONFIGURATION - 3-Level Quality System for Layout 5 (6 Photos)
    ...
    // 🎯 LAYOUT 5 CONFIGURATION - 6 PHOTOS
    const expectedPhotos = 6; // Layout 5 memiliki 6 foto
    let images = [];
    let invertBtnState = false;
    let currentImageIndex = 0;

    // 🚀 FAST COMPRESSION FUNCT    console.log(`🎯 Layout 5 (${expectedPhotos} photos) initialized successfully!`);ON
    ...
    if (window.location.pathname.includes("canvasLayout5.php")) {
        startCamera();
    }
    ...
    async function startPhotobooth() {
        ...
        try {
            ...
            const originalKey = `canvasLayout5_original_${images.length - 1}`;
    ...
    window.retakeSinglePhoto = function(index) {
        if (index >= 0 && index < images.length) {
            ...
            const originalKey = `canvasLayout5_original_${index}`;
            localStorage.removeItem(originalKey);
            
            for (let i = index; i < expectedPhotos - 1; i++) {
                const oldKey = `canvasLayout5_original_${i + 1}`;
                const newKey = `canvasLayout5_original_${i}`;
    ...
    async function storeImageArray() {
        ...
        try {
            sessionStorage.setItem('canvasLayout5_images', JSON.stringify(images));
            
            for (let i = 0; i < images.length; i++) {
                const originalKey = `canvasLayout5_original_${i}`;
                if (!localStorage.getItem(originalKey)) {
                    localStorage.setItem(originalKey, images[i]);
                }
            }
            
            console.log(`✅ Layout 3: Stored ${images.length} photos successfully`);
        ...
        retakeAllBtn.addEventListener('click', () => {
            if (confirm(`Are you sure you want to retake all ${expectedPhotos} photos?`)) {
                ...
                for (let i = 0; i < expectedPhotos; i++) {
                    localStorage.removeItem(`canvasLayout5_original_${i}`);
                }
    ...
    console.log(`🎯 Layout 3 (${expectedPhotos} photos) initialized successfully!`);
});
