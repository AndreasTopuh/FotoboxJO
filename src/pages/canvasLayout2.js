document.addEventListener('DOMContentLoaded', () => {
    // 🚀 COMPRESSION CONFIGURATION - ENHANCED & SYNCHRONIZED Quality System for Layout 2
    // ✅ SYNCHRONIZED dengan customizeLayout2.js untuk konsistensi kualitas end-to-end
    const COMPRESSION_CONFIG = {
        // Untuk session storage (temporary) - HIGH QUALITY untuk hasil yang konsisten
        SESSION_QUALITY: 0.9,        // 90% - selaras dengan Layout 1 & customizeLayout2.js
        ...
        DOWNLOAD_QUALITY: 0.98,      // 98% - selaras dengan Layout 1 (FIXED: was 1.05!)
        ...
        THUMB_MAX_WIDTH: 600,        // Naikan selaras dengan Layout 1
        THUMB_MAX_HEIGHT: 800        // Naikan selaras dengan Layout 1
    };
    ...
    // Asset Loading - Layout 2 specific
    async function loadAssetsFromDatabase() {
        ...
        try {
            ...
            // Load layout 2 specific stickers
            if (stickersData.success && stickersData.data) {
                loadDynamicStickers(stickersData.data);
            }

            // Load layout 2 specific frame & sticker combos
            if (comboData.success && comboData.data) {
                loadDynamicFrameStickerCombos(comboData.data);
            }

            console.log(`✅ Loaded Layout 2 assets successfully`);
        } catch (error) {
            console.error('❌ Error loading Layout 2 assets from database:', error);
        }
    }
    ...
    // Dynamic sticker loading (layout 2 specific)
    function loadDynamicStickers(stickers) {
        ...
        console.log(`✅ Created ${stickers.length} dynamic sticker controls for Layout 2`);
    }

    // Dynamic frame & sticker combo loading (layout 2 specific)
    function loadDynamicFrameStickerCombos(combos) {
        ...
        console.log(`✅ Created ${combos.length} dynamic frame & sticker combo controls for Layout 2`);
    }

    // Initialize Layout 2 assets on page load
