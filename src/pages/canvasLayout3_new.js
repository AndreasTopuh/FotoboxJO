document.addEventListener('DOMContentLoaded', () => {
    // 🚀 COMPRESSION CONFIGURATION - 3-Level Quality System for Layout 3 (6 Photos)
    ...
    // 🎯 LAYOUT 3 CONFIGURATION - 6 PHOTOS
    const expectedPhotos = 6; // Layout 3 memiliki 6 foto
    let images = [];
    ...
    async function startPhotobooth() {
        ...
        try {
            ...
            const originalKey = `canvasLayout3_original_${images.length - 1}`;
    ...
    window.retakeSinglePhoto = function(index) {
        if (index >= 0 && index < images.length) {
            ...
            const originalKey = `canvasLayout3_original_${index}`;
            localStorage.removeItem(originalKey);
            
            for (let i = index; i < expectedPhotos - 1; i++) {
                const oldKey = `canvasLayout3_original_${i + 1}`;
                const newKey = `canvasLayout3_original_${i}`;
    ...
    async function storeImageArray() {
        ...
        try {
            sessionStorage.setItem('canvasLayout3_images', JSON.stringify(images));
            
            for (let i = 0; i < images.length; i++) {
                const originalKey = `canvasLayout3_original_${i}`;
        ...
        retakeAllBtn.addEventListener('click', () => {
            if (confirm(`Are you sure you want to retake all ${expectedPhotos} photos?`)) {
                ...
                for (let i = 0; i < expectedPhotos; i++) {
                    localStorage.removeItem(`canvasLayout3_original_${i}`);
                }
