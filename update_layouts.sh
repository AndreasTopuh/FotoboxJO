...
echo "🔄 Updating customizeLayout4-6 with database integration..."
...
for i in {4..6}; do
    ...
    # Update frames container
    sed -i '/Frame Color/,/\/div>/{
        s/<div class="customize-buttons-grid">/<div class="customize-buttons-grid" id="frames-container">/
        /<button.*Frame/,/<\/button>/d
        /id="frames-container">/a\                    <!-- Dynamic frames will be loaded here -->\n                    <div class="loading-placeholder">Loading frames...</div>
    }' src/pages/customizeLayout${i}.php
    
    # Update stickers container  
    sed -i '/Stickers/,/\/div>/{
        s/<div class="customize-buttons-grid stickers-grid">/<div class="customize-buttons-grid stickers-grid" id="stickers-container">/
        /<button.*Sticker/,/<\/button>/d
        /id="stickers-container">/a\                    <!-- Dynamic stickers will be loaded here -->\n                    <div class="loading-placeholder">Loading stickers...</div>
    }' src/pages/customizeLayout${i}.php
    
    # Add assets manager script
...
EOF
    ...
    # Remove old initializeStickerControls function
